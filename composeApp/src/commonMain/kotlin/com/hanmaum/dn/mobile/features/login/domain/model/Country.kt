...
/**
 * A dialable country. [flag] is computed from the ISO code (regional indicator
 * symbols) so we don't store emoji literals. [dialCode] excludes the leading '+'.
 */
data class Country(
    val iso: String,
    val name: String,
    ...
    val flag: String get() = isoToFlag(iso)
}

/** Converts a 2-letter ISO country code to its flag emoji (e.g. "DE" -> 🇩🇪). */
fun isoToFlag(iso: String): String {
    if (iso.length != 2) return ""
    return buildString {
        for (c in iso.uppercase()) {
            val cp = 0x1F1E6 + (c.code - 'A'.code) // regional indicator base
            val off = cp - 0x10000
            append((0xD800 + (off shr 10)).toChar())
            append((0xDC00 + (off and 0x3FF)).toChar())
        }
    }
}

object Countries {
    const val DEFAULT_ISO = "DE"
    ...
    fun byIsoOrDefault(iso: String?): Country =
        byIso[iso?.uppercase()] ?: byIso.getValue(DEFAULT_ISO)

    /** Case-insensitive search over name and dial code (with or without '+'). */
...
/** Pure helpers for assembling/sanitizing phone input — no platform deps. */
object PhoneNumber {
    fun sanitizeNational(input: String): String = input.filter { it.isDigit() }

    /** Combines a dial code + national digits into E.164, or null when empty. */
    fun toE164(dialCode: String, national: String): String? {
        val digits = sanitizeNational(national)
        return if (digits.isEmpty()) null else "+$dialCode$digits"
    }
}
