...
| iOS launches then crashes: `IrLinkageError ... $stable ... backing field` | lifecycle / navigation-compose / koin compiled against different lifecycle lines | Align all three (check the candidate's `.module` on Maven); `iosSimulatorArm64Test` before any build, runtime on the user's TestFlight — start the local simulator only when the user asks |
...
| Archive dies in `Compile Kotlin Framework` with `Compilation failed: Java heap space` | `gradle.properties` asks for 4G Gradle + 3G Kotlin daemon = the whole 7G of a macos-15-arm64 runner, so the K/N linker never gets its ceiling | Set `GRADLE_OPTS` on the iOS lanes only (more for Gradle, less for the idle Kotlin daemon). Confirm it is contention, not appetite, by running `linkReleaseFrameworkIosArm64` locally pinned to the same `-Xmx` |
