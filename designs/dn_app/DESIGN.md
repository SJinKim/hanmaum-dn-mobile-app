# Design System Specification: DN App
...
*   **Primary (`#ae2f34`):** Use this deep Coral tone for high-energy moments and primary actions. It represents the "heart" of the community.
*   **Secondary (`#005db8`):** Faith Blue provides a grounding, trustworthy foundation for navigation and secondary elements.
*   **Tertiary (`#705d00`):** Holy Gold is reserved for "Aha!" moments, inspirational highlights, and achievement states.
*   **Neutral (`#FAFAFA`):** A soft, light background for surfaces.
...
### Cards
*   **Structure:** No dividers. Use `rounded-xl` (3rem) for large container cards and `rounded-lg` (2rem) for internal cards. Given the roundedness value of 3 in the token, which implies maximum pill-shaped, this aligns well with the design intent for very rounded corners.
*   **Padding:** Aggressive internal padding (32px-48px) to reinforce the high-end editorial feel. This complements the updated `spacing` token which is set to `2` (Normal), allowing for deliberate padding choices at the component level within a generally balanced spacing system.
...
*   **Do** prioritize "Reading Time" and "Whitespace." If a screen feels "full," remove an element. The `spacing` token being `2` (normal) provides a good foundation for this, allowing for intentional whitespace without being overly sparse.
...
*   **Don’t** use sharp 90-degree corners. Everything in this system must feel soft and approachable, which is reflected in the `roundedness` token being set to `3` (Maximum, pill-shaped).
